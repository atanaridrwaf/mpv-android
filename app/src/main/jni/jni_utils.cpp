...
void init_methods_cache(JNIEnv *env)
{
    ...
    mpv_MPVLib_eventCommandReply_li = env->GetStaticMethodID(mpv_MPVLib, "eventCommandReply", "(JI)V"); // eventCommandReply(long, int)
    mpv_MPVLib_eventHook_S = env->GetStaticMethodID(mpv_MPVLib, "eventHook", "(Ljava/lang/String;)V"); // eventHook(String)
    mpv_MPVLib_event = env->GetStaticMethodID(mpv_MPVLib, "event", "(I)V"); // event(int)
    mpv_MPVLib_eventEndFile_b = env->GetStaticMethodID(mpv_MPVLib, "eventEndFile", "(Z)V"); // eventEndFile(boolean)
